...
WITH uitesting_scd2_timestamp AS (
...
Join_1_3 AS (
  ...
  INNER JOIN SQLStatement31 AS in1
...
distinct_rows_by_partition AS (
  ...
  QUALIFY ROW_NUMBER() OVER (PARTITION BY concat(c_struct.city, c_struct.state), c_boolean, c_struct.state ORDER BY c_int ASC, c_bigint DESC) = 1
...
parent_transform_deduplicate_1 AS (
  ...
  {{
    SQL_DatabricksParentProjectMain.parent_transform_deduplicate(
      relation = 'env_uitesting_main_model_databricks_1', 
      partition_by = 'p_string', 
      order_by = 'c_string'
    )
  }}
...
Subgraph_10 AS (
  ...
  Subgraph_8 AS (
    ...
    WITH all_type_partitioned_1_1 AS (
...
DataCleansing_1_1 AS (
  ...
  {{
    DatabricksSqlBasics.DataCleansing(
      ...
      [
        { "name": "c_int_new", "dataType": "String" }, 
        { "name": "c_float_new", "dataType": "String" }, 
        { "name": "c_string_new", "dataType": "String" }, 
        { "name": "c_boolean_new", "dataType": "String" }, 
        { "name": "c_struct_new", "dataType": "Struct" }
      ], 
      'makeLowercase', 
      ['c_int_new', 'c_float_new', 'c_string_new', 'c_boolean_new'], 
      true, 
      'NA', 
      true, 
      0, 
...
combine_multiple_tables_1 AS (
  ...
  {{
    SQL_DatabricksSharedBasic.combine_multiple_tables(
      table_1 = 'Limit_2', 
      table_2 = 'Limit_5', 
      table_3 = 'child_deduplicate_custom_1', 
      table_4 = 'Limit_3', 
